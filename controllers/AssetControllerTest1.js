...
AssetController.prototype.getTS = function(req, callback) {
...
//console.log(' [INFO] Bearer ' + uaaControllerTS.getTokenTS() );    // -> '02a2ce90-1432-11e1-8558-0b488e4fc115')
 ...
 body: { start: '1y-ago', tags: [ { name: req.params.name } ] },
 json: true };
...
// //////
AssetController.prototype.postKitAsset = function(req, callback) {
  var options = { method: 'POST',
  ...
  headers: 
   { 
     ...
     'predix-zone-id': '8b3945d0-b3d9-487f-acec-c6a58e5d6b53' },
  body: req.body };
...
// /////
AssetController.prototype.getAssetsTEST = function(req, callback) {
  console.log("[INFO] Fetching asset...(B TOKEN = ) ************ ");
  var self = this;
...
console.log("HERE: " + config.asset.ingestUri+'/'+ req.params.type + '/' + req.params.model);
  ...
  request.get(options, function (error, response, body) {
    if (!error && response.statusCode == 200) {
      ...
      self.getTSdEMO(self.model[0].uri,function(temp){
...
AssetController.prototype.getTSdEMO = function(m, callback) {
  console.log("[INFO] Fetching asset... ");
  var self = this;
